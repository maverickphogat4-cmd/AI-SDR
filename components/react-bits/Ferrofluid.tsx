...
const Ferrofluid: React.FC<FerrofluidProps> = ({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      // Capped at 1.5, not the real devicePixelRatio, when the caller
      // doesn't pass its own `dpr` -- this runs continuously as a page
      // background alongside another WebGL canvas (Topography) on the
      // dashboard, right behind a form people are actively typing into, so
      // retina-DPR cost isn't worth it here (same reasoning as the
      // Topography DPR edit).
